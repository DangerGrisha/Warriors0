...
// =========================
    ...
    public void onSoulReleaseUse(PlayerInteractEvent e) {
        ...
        if (!soulHand) {
            p.sendMessage("§c[ULT-DBG] STOP: e.getItem() is not Soul Release");
            return;
        }

        e.setCancelled(true);
        p.sendMessage("§a[ULT-DBG] passed Soul Release check");

        if (!isSaberLight(p)) {
            p.sendMessage("§c[ULT-DBG] STOP: no LightSaber tag");
            p.sendActionBar(Component.text("§cOnly LightSaber can use this."));
            return;
        }
        p.sendMessage("§a[ULT-DBG] passed LightSaber tag");
        ...
        p.sendMessage("§a[ULT-DBG] passed Excalibur main-hand check");
    ...
    public void onSoulReleaseCrossbowShoot(EntityShootBowEvent e) {
        ...
        p.sendMessage("§8[ULT-DBG] Crossbow shot cancelled (Soul Release).");
    }
