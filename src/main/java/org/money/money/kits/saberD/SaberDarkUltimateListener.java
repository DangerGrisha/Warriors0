...
public final class SaberDarkUltimateListener implements Listener {

    private final Plugin plugin;
    private final SaberDarkExcaliburListener excalibur; // reuse your soul methods (recommended)
...
// =========================
    ...
    public SaberDarkUltimateListener(Plugin plugin, SaberDarkExcaliburListener excalibur) {
        this.plugin = plugin;
    ...
    /* =========================
       Activation: Blue Dye "Soul ReleaseD"
       ========================= */
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (ticks >= CAST_LOCK_TICKS) {
                    ...
                    w.spawnParticle(Particle.SMOKE, c, 3, 0.25, 0.25, 0.25, 0.0);
    ...
    private void playMaxHeightSkyFlare(Player p, CastState state) {
        ...
        w.spawnParticle(Particle.SMOKE, sky, 4, 0.45, 0.25, 0.45, 0.0);
        w.spawnParticle(Particle.EXPLOSION, sky, 5, 0.70, 0.35, 0.70, 0.0);
        w.spawnParticle(Particle.SQUID_INK, sky, 24, 0.9, 0.6, 0.9, 0.02);
        w.spawnParticle(Particle.ELECTRIC_SPARK, sky, 36, 0.85, 0.55, 0.85, 0.08);
        w.spawnParticle(Particle.CRIMSON_SPORE, sky, 20, 0.75, 0.45, 0.75, 0.03);
    ...
    private void drawCasterReleaseDispersal(Player p, int localTick) {
        ...
        for (int i = 0; i < rays; i++) {
            ...
            for (int t = 1; t <= trail; t++) {
                ...
                w.spawnParticle(Particle.CRIMSON_SPORE, pt, 1, 0.02, 0.02, 0.02, 0.0);
                if ((i + t + localTick) % 2 == 0) {
                    w.spawnParticle(Particle.SQUID_INK, pt, 1, 0.01, 0.01, 0.01, 0.0);
                } else {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                for (int i = 0; i <= steps; i++) {
                    ...
                    w.spawnParticle(Particle.SQUID_INK, p, 2, 0.01, 0.01, 0.01, 0.0);
                ...
                for (int i = -cutPoints; i <= cutPoints; i++) {
                    ...
                    w.spawnParticle(Particle.CRIMSON_SPORE, edge, 1, 0.03, 0.02, 0.03, 0.0);
                    if ((i + t) % 2 == 0) {
                ...
                if (t >= DROP_TICKS) {
                    w.spawnParticle(Particle.SMOKE, groundImpact, 5, 0.35, 0.25, 0.35, 0.0);
    ...
    private void drawFallingBeamPillar(World w, Location impact, int tick, double pillarHeight) {
        ...
        for (int i = 0; i <= verticalSteps; i++) {
            ...
            // Bright core line
            w.spawnParticle(Particle.SQUID_INK, pt, 2, 0.03, 0.03, 0.03, 0.0);
        ...
        w.spawnParticle(Particle.SMOKE, c, 1, 0.18 * PHASE4_BLAST_SIZE, 0.08, 0.18 * PHASE4_BLAST_SIZE, 0.0);
    ...
    private void finalImpactCinematic(Player caster, Location target, Vector lockedDir) {
        ...
        w.spawnParticle(Particle.SMOKE, target, 8, 1.4, 0.7, 1.4, 0.0);
        ...
        w.spawnParticle(Particle.CRIMSON_SPORE, target, 100, 2.2, 1.0, 2.2, 0.08);
    ...
    private void drawChargeAura(Player p) {
        ...
        w.spawnParticle(Particle.SQUID_INK, l, 10, 0.35, 0.4, 0.35, 0.01);
        w.spawnParticle(Particle.ELECTRIC_SPARK, l, 8, 0.30, 0.35, 0.30, 0.04);
        w.spawnParticle(Particle.CRIMSON_SPORE, l, 4, 0.20, 0.25, 0.20, 0.01);
        ...
        for (int i = 1; i <= 6; i++) {
            ...
            w.spawnParticle(Particle.SQUID_INK, pt, 1, 0, 0, 0, 0);
        }
    }
    private void drawGoldenChargeRings(Player p, int tick) {
        ...
        for (int i = 0; i < points; i++) {
            ...
            if (i % 5 == 0) {
                w.spawnParticle(Particle.SQUID_INK, p1, 1, 0.01, 0.01, 0.01, 0.0);
            }
        }
    }
    ...
    private void drawRisingSparks(Player p, int tick) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (tick % 8 == 0) w.spawnParticle(Particle.SQUID_INK, pt, 1, 0, 0, 0, 0);
        }
    }
    ...
    private void drawAscendingExcaliburColumn(Player p, int localTick, int phaseTicks) {
        ...
        for (int i = 0; i <= coreSteps; i++) {
            ...
            if (i % 2 == 0) w.spawnParticle(Particle.SQUID_INK, pt, 1, 0.01, 0.01, 0.01, 0.0);
            if (i % 3 == 0) w.spawnParticle(Particle.CRIMSON_SPORE, pt, 1, 0.03, 0.03, 0.03, 0.0);
        }
    ...
    private void drawOverloadBurst(Player p, int localTick) {
        ...
        if (localTick > 40 && localTick % 8 == 0) {
            w.spawnParticle(Particle.SMOKE, center, 1, 0.15, 0.15, 0.15, 0.0);
        }
    }
    ...
    private void drawSlashBeamFrame(World w, Location head, Vector beamStepDir, Vector lockedLookDir, int tick) {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            w.spawnParticle(Particle.SQUID_INK, pt, 4, 0.10, 0.10, 0.10, 0.01);
            w.spawnParticle(Particle.CRIMSON_SPORE, pt, 3, 0.08, 0.08, 0.08, 0.01);
        ...
        // 3) Random flash pops
        if (tick % 4 == 0) {
            w.spawnParticle(Particle.SMOKE, head, 1, 0.2, 0.2, 0.2, 0.0);
        }
    }
    ...
    private void spawnGoldParticle(World w, Location loc, float size) {
        // Blood red dust look
        Particle.DustOptions gold = new Particle.DustOptions(Color.fromRGB(170, 0, 0), size);
    ...
    private void drawBeamSegment(World w, Location head, Vector beamDir, double radius) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            w.spawnParticle(Particle.CRIMSON_SPORE, p, 3, 0.08, 0.08, 0.08, 0.01);
            w.spawnParticle(Particle.SQUID_INK, p, 5, 0.18, 0.18, 0.18, 0.01);
    ...
    private boolean isSoulRelease(ItemStack it) {
        ...
        if (plain == null || !plain.equalsIgnoreCase("Soul ReleaseD")) return false;
    ...
    /**
     * Consumes souls directly from ExcaliburD in main hand by rebuilding item with reduced soul count.
    ...
    private boolean consumeSoulsFromMainHandExcalibur(Player p, int cost) {
        ...
        // --- IMPORTANT ---
        // Add a public method in SaberDarkExcaliburListener:
    ...
    public static ItemStack makeSoulReleaseCrossbow() {
        ...
        meta.displayName(Component.text("Soul ReleaseD"));
