...
public final class SaberDarkExcaliburListener implements Listener {
    ...
    private static double dashPower() { return org.money.money.meta.ClassRegistry.num("saberdark", "excalibur", "dashPower", 1.4); }
    private static int dashCooldownSeconds() { return org.money.money.meta.ClassRegistry.numInt("saberdark", "excalibur", "dashCooldownSeconds", 7); }
    ...
    private void dash(Player p) {
        ...
        w.playSound(p.getLocation(), Sound.ENTITY_PLAYER_ATTACK_SWEEP, 1.0f, 0.8f);
        w.playSound(p.getLocation(), Sound.PARTICLE_SOUL_ESCAPE, 1.0f, 0.7f);
        w.spawnParticle(Particle.SMOKE, at, 20, 0.2, 0.15, 0.2, 0.03);
        w.spawnParticle(Particle.SOUL_FIRE_FLAME, at, 12, 0.2, 0.1, 0.2, 0.02);
